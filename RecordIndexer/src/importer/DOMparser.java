...
public class DOMparser {
	...
	public static void main(String[] args) throws Exception{
		...
		if(args == null || args.length == 0){
			...
			prefix = file.getParentFile().getPath() + "/";
		}
	...
	public static ArrayList<Field> parseField(NodeList fields){
		for(int i = 0; i < fields.getLength(); i++){
			...
			String helphtml = prefix + helphtmlElem.getTextContent();
			String knowndata = null;
			if(knowndataElem != null){
				knowndata = prefix + knowndataElem.getTextContent();
			}
		...
		db.startTransaction();
		ArrayList<Field> fieldsObjects = db.getFields().getFieldsByProjectID(projectID);
		db.endTransaction(true);
	...
	public static void parseImage(NodeList images, ArrayList<Field> fieldsObjects){
		for(int i = 0; i < images.getLength(); i++){
			...
			String file = prefix + fileElem.getTextContent();
